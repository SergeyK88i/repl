...
Для MVP LangGraph не является обязательной зависимостью.

Начальный подход к reasoning должен быть проще и контролируемее:
...
LangGraph можно добавить позже точечно внутри конкретного агента, но он не должен подменять contracts, ports, adapters, state machine и trace.
...
## Reasoning, контекст и память

Reasoning-слой LLM-агента должен быть явным модулем, а не скрытым prompt-скриптом.

Для LLM-агентов допускается папка:
...
Reasoning service должен возвращать структурированный результат, который application service может проверить до выполнения tools.

Reasoning не должен:

- менять статус;
- напрямую вызывать adapters;
- напрямую писать в repositories;
- выполнять внешние вызовы без typed tools;
- использовать prompt history как источник истины.

Контекст LLM должен собираться через `context_builder` из устойчивого состояния:

- preorder/order state;
- task state;
- agent run state;
- trace summary;
- tool results;
...
- execution context — обязательные идентификаторы текущего запуска;
- short-term memory — состояние текущего agent run и выполненных шагов;
- long-term memory — knowledge base, runbooks, успешные remediation-паттерны;
- conversation memory — история общения с пользователем.

Long-term memory и conversation memory не являются authority. Они могут помогать reasoning, но не могут обходить WARP, СДО, state machine, permissions и contracts.

Reasoning endpoint-ы не должны быть публичным бизнес API по умолчанию.

Если нужны endpoint-ы для отладки reasoning, они должны быть внутренними:

```text
POST /internal/{agent}/reasoning/preview
```

Они не должны выполнять tools или менять состояние.

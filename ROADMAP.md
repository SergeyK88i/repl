...
- что после `READY` стартует ЕР-Конфигуратор;
...
- Создать `src/agents/ep_configurator/`.
- Заменить `MockReplicaInitAdapter` на `EpConfiguratorAdapter`.
...
- Добавить статусы после READY:
  ...
  - `FAILED`;
  - `ESCALATED`.
- Добавить trace-события ЕР-Конфигуратора.

### Критерии готовности

- После READY Координатор передаёт задачу ЕР-Конфигуратору.
- Создаётся заказ в нужном Jira-пространстве.
- Формируется конфиг для ЕР.
- Ошибки конфигурации эскалируются с контекстом.

## Фаза 6. Init Loader и Публикатор
...
3. Подключить настоящий WARP readiness
4. Реализовать CR Manager
5. Подключить Jira и remediation
6. Реализовать Trace Collector
7. Реализовать ЕР-Конфигуратор
8. Добавить Init Loader и Публикатор
9. Добавить LLM reasoning там, где есть ценность
10. Провести production hardening
```

...
→ если источник не готов, Координатор создаёт remediation-поручение для CR Manager
...
→ Координатор создаёт remediation-поручение для CR Manager
→ предзаказ переходит в WAITING_CR
```

В целевой production-архитектуре Jira/CR-задачу создаёт уже CR Manager. Координатор только фиксирует, что источнику нужна remediation-работа, и передаёт CR Manager контекст: источник, план загрузки, `warp_check_id`, невыполненные критерии и параметры.
